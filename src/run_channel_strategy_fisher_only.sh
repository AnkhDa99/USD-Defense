...
OUTPUT_ROOT=${OUTPUT_ROOT:-./save/channel_strategy_fisher_only}
NUM_CLASS=${NUM_CLASS:-10}

# 适配你当前这份 Remove_Backdoor_FIP0.py
EPOCH=${EPOCH:-2000}
...
run_one() {
  ...
  echo "[RUN] mode=Fisher-only (no USD)"
...
SUMMARY_CSV="$OUTPUT_ROOT/${DATASET}_${ARCH}_${ATTACK}_fisher_only_summary.csv"
SUMMARY_MD="$OUTPUT_ROOT/${DATASET}_${ARCH}_${ATTACK}_fisher_only_summary.md"
SUMMARY_TXT="$OUTPUT_ROOT/${DATASET}_${ARCH}_${ATTACK}_fisher_only_summary.txt"
...
mapping = [
    ('random', 'Random+Fisher'),
    ('high_response', 'High-response+Fisher'),
    ('ours', 'Ours(RC)+Fisher'),
]
...
lines = []
lines.append(f"表 X {dataset}数据集下不同通道选择策略在Fisher-only设置下的净化结果")
lines.append(f"Table X Purification results under different channel selection strategies with Fisher-only on {dataset}")
lines.append("")
lines.append("| Strategy | ACC(%) | ASR(%) |")
lines.append("|---|---:|---:|")
for _, r in df.iterrows():
    lines.append(f"| {r['Strategy']} | {r['ACC(%)']} | {r['ASR(%)']} |")
