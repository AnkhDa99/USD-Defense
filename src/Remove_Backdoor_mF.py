...
from zyh.FIP.src.causal_analyzer_De1 import CausalAnalyzer
import models
...
def main(parser, transform_train, transform_test):
    ...
    if args.poison_type in ['badnets', 'blend']:
        ...
        poisoned_data = Dataset_Feature_npy(np.load(args.poisoned_data_test_all2one, allow_pickle=True), mode='test',
    ...
    net = getattr(networks, args.arch)(num_classes=10).to(device)  ## For Mask-finetuning
    ...
    if args.targeted:
        ...
        soda_analyzer = CausalAnalyzer(
            ...
            ca_alpha=args.soda_ca_alpha, ca_beta=args.soda_ca_beta,
            pcc_th=args.soda_pcc_th, mad_th=args.soda_mad_th
        )

        # 2. 决定使用SODA自动检测结果还是手动指定参数
        if args.skip_soda_detection:
            print("--- Skipping SODA auto-detection as requested. Using command-line target/source. ---")
            # 在这种模式下，程序将直接使用您在命令行中为 --target_label 和 --poison_source 设置的值
        else:
            print("\n--- SODA Eye: Attempting Full Automatic Detection ---")
            detection_result = soda_analyzer.run_full_detection(args.data_dir, args.batch_size)
            
            # --- 关键修改：检查SODA结果，如果失败则回退，而不是中断 ---
            if detection_result.get('is_backdoored') and detection_result.get('target_class') is not None and detection_result.get('source_class') is not None:
                # 如果SODA成功找到了明确的源和目标，则使用SODA的结果
                detected_target = detection_result.get('target_class')
                detected_source = detection_result.get('source_class')
                print(f"--- SODA detection successful. Overriding target to {detected_target} and source to {detected_source}. ---")
                args.target_label = detected_target
                args.poison_source = detected_source
            else:
                # 如果SODA报告模型干净或无法找到明确目标，则打印警告并使用命令行参数
                print(f"--- SODA detection inconclusive. Falling back to command-line specified target={args.target_label} and source={args.poison_source}. ---")
        
        # 3. 使用最终确定的参数进行靶向净化
        guilty_param_names = soda_analyzer.get_target_layer_param_names()
        if not guilty_param_names:
            print("Error: Could not identify target parameters for purification. Aborting.")
            return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--poison-type', type=str, default='badnets',
                        choices=['badnets', 'Feature', 'FC', 'SIG', 'Dynamic', 'TrojanNet', 'blend', 'CLB', 'benign','semantic2'],
    ...
    parser.add_argument("--reg_F", default=0.005, type=float, help="CDA Regularizer Coefficient, eta_F")

    parser.add_argument('--targeted', action='store_true', help='Enable targeted purification instead of global FIP.')
    parser.add_argument('--soda_ana_layer', type=int, default=4, help='Layer index for SODA analysis in targeted mode.')
