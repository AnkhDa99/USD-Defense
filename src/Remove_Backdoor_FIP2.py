...
# from GradientRegularizer import GradientRegularizer as regularizer   ## Regularizer

# from PIMRegularizerMixin import CDA_Regularizer as regularizer

from FastBackdoorRemover import FastBackdoorRemover
...
def main(parser, transform_train, transform_test):
    ...
    # 1
    # criterion_reg = regularizer(args, device, net, criterion, lr=args.lr)
    # 2
    #criterion_reg = regularizer(args, device, net, criterion, lr=args.lr, sample_ratio=0.3)

    # 混合
    # criterion_reg = regularizer(
    #     args=args,
    #     device=device,
    #     model=net,
    #     criterion=criterion,
    ...
    #     grad_weight=0.5
    # )
    # criterion_reg.register_ewc_params(clean_val, batch_size=128, num_batches=100)

    # 一阶泰勒
    # criterion_reg = regularizer(args, device, net, criterion, lr=args.lr)

    #第四种
    # criterion_reg = GradientRegularizer(args, device, model=net, criterion=criterion, lr=args.lr)
    # criterion_reg.register_fisher_info(clean_val, batch_size=128, num_batches=100)

    # lightweighter
    # criterion_reg = regularizer(args, device, net, criterion, lr=args.lr)

    # criterion_reg = FastBackdoorRemover(args, device, net, criterion)
    # criterion_reg.update_anchors(clean_val)  # 使用干净样本初始化

    # 初始化 remover
    criterion_reg = FastBackdoorRemover(args=args, device=device, model=net, criterion=criterion)
    criterion_reg.update_anchors(clean_val_loader, num_batches=10)

    # # 初始化正则器（net 是你的模型，criterion 是交叉熵损失函数等）
    # criterion_reg = FastCDAHybrid(args=args, device=device, model=net, criterion=criterion)
    # # 注册 Fisher 信息和 anchor 参数（使用干净验证集）
    # criterion_reg.update_fisher(clean_val_loader, num_batches=10)
...
def FIP_Train(args, epoch, net, clean_val, clean_val_loader, criterion_reg):
    ...
    # tqdm描述初始化
    desc = '[Fisher][LR=%s] Loss: 0.000 | Acc: 0.000%% (0/0)' % args.lr
    prog_bar = tqdm(enumerate(clean_val_loader), total=len(clean_val_loader), desc=desc, leave=True)
    ...
    for batch_idx, (inputs, targets) in prog_bar:
        ...
        optimizer = None
        if hasattr(criterion_reg, 'model') and hasattr(criterion_reg.model, 'optimizer'):
            optimizer = criterion_reg.model.optimizer
        else:
            optimizer = torch.optim.SGD(net.parameters(), lr=args.lr)
        optimizer.zero_grad()

        # 这里示范每2个batch调用一次forward_backward_update
        if batch_idx % 2 == 0 and hasattr(criterion_reg, 'forward_backward_update'):
            loss, outputs = criterion_reg.forward_backward_update(inputs, targets, batch_idx)
        else:
            with torch.no_grad():
                outputs = net(inputs)
        ...
        # 更新进度条描述，tqdm 会自动计算速度显示 it/s
        desc = '[Fisher][LR=%s] Loss: %.3f | Acc: %.3f%% (%d/%d)' % (
            args.lr, train_loss / (batch_idx + 1), 100. * correct / total, correct, total)
        prog_bar.set_description(desc, refresh=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gamma', type=float, default=0.1, help='EWC强度')
